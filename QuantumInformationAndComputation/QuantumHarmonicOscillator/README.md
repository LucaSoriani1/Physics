...
In the code, we set $\hbar = 1$ and the solution of the eigenvalues is obtained through the numerical solution of the time-dependent Schrödinger equation, which is rewritten as a position-dependent eigenvalue problem. In particular, a reduced-banded symmetric tridiagonal matrix representing the harmonic potential is created, and the eigenvalues and eigenfunctions are calculated using the eigh_tridiagonal function of the scipy.linalg module. Finally, the graph of the eigenfunctions and eigenvalues of the system is displayed.
